...
extension URLRequest {
  static func postWith(url: URL, data: [String: String]) -> URLRequest {
    return NSMutableURLRequest(url: url).apply {
      $0.httpMethod = HTTPMethod.post.rawValue
      $0.httpBody = data.jsonData
    }.copy() as! URLRequest
  }
}
...
extension Dictionary {
  ...
  var jsonValue: String {
    return self.jsonData?.stringValue ?? "{}"
  }
}
